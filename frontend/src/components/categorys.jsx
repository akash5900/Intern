            ...
            {category.map((cat) => (
                <div className=" border border-gray-400 hover:shadow-lg rounded w-[190px] h-[210px] hover:bg-white flex flex-col items-center gap-3 p-2 mx-6 cursor-pointer" key={cat._id} >
                    ...
                    <div className="flex gap-5">
                        <Link to={`/updatecategory/${cat._id}`} className="border rounded cursor-pointer px-1 hover:bg-red-400">Edit</Link>
                        <button className="border rounded cursor-pointer px-1 hover:bg-blue-400" onClick={() => handleDelete(cat._id)}>Delete</button>
                    </div>
                </div>
            ))}
